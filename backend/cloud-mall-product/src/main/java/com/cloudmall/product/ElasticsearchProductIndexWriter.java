...
public class ElasticsearchProductIndexWriter implements ProductIndexWriter {
    ...
    public ElasticsearchProductIndexWriter(RestHighLevelClient client, ObjectMapper objectMapper,
    ...
    public void upsert(ProductIndexDocument document) throws Exception {
        ...
        client.index(new IndexRequest(indexName).id(String.valueOf(document.id()))
    ...
    public void delete(long productId) throws Exception {
        try {
            client.delete(new DeleteRequest(indexName, String.valueOf(productId)), RequestOptions.DEFAULT);
