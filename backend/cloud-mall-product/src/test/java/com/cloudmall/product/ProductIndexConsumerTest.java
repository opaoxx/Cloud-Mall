...
class ProductIndexConsumerTest {
    ...
    void changedEventReadsMySqlAndUpsertsDeterministicDocument() throws Exception {
        ProductIndexDocument document = new ProductIndexDocument(42L, 3L, "键盘", null, "desc", new BigDecimal("99.00"), 1);
    ...
    void categoryEventStillRequiresAValidEnvelope() throws Exception {
        byte[] malformed = mapper.writeValueAsBytes(Map.of(
                "eventId", "event-1", "eventType", "CATEGORY_CHANGED",
                "traceId", "trace-1", "payload", Map.of("categoryId", 42L)));
    ...
    void productEventRejectsMissingOrInconsistentEnvelopeFields() throws Exception {
        byte[] missingOccurredAt = mapper.writeValueAsBytes(Map.of(
                "eventId", "event-1", "eventType", "PRODUCT_CHANGED", "traceId", "trace-1",
                "businessKey", Map.of("name", "productId", "value", 42L),
                "payload", Map.of("productId", 42L), "productId", 42L));
        byte[] inconsistentPayload = mapper.writeValueAsBytes(Map.of(
                "eventId", "event-1", "eventType", "PRODUCT_CHANGED", "occurredAt", "2026-09-04T12:00:00+08:00",
                "traceId", "trace-1", "businessKey", Map.of("name", "productId", "value", 42L),
                "payload", Map.of("productId", 43L), "productId", 42L));
    ...
    void malformedOrFailedEventIsNotSwallowed() throws Exception {
        ...
        assertThrows(IllegalStateException.class, () -> consumer.consume(event("PRODUCT_CHANGED", 42L)));
    }

    private byte[] event(String type, long productId) throws Exception {
        return mapper.writeValueAsBytes(Map.of("eventId", "event-1", "eventType", type,
                "occurredAt", "2026-09-04T12:00:00+08:00", "productId", productId,
                "businessKey", Map.of("name", "productId", "value", productId),
                "traceId", "trace-1", "payload", Map.of("productId", productId)));
    }

    private byte[] categoryEvent(String type, long categoryId) throws Exception {
        return mapper.writeValueAsBytes(Map.of("eventId", "event-1", "eventType", type,
                "occurredAt", "2026-09-04T12:00:00+08:00", "categoryId", categoryId,
                "businessKey", Map.of("name", "categoryId", "value", categoryId),
                "traceId", "trace-1", "payload", Map.of("categoryId", categoryId)));
    }
}
